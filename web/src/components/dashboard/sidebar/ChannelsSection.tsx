  ...
  return (
    <li className="px-8 py-2 flex justify-between items-center cursor-pointer group">
      <div className="flex items-center group-hover:w-4/6 w-full">
        ...
        >
          <HashtagIcon className="h-4 w-4" style={{ color: "#4a154b" }} />
        </div>
        <span className="text-sm th-color-for truncate">
      ...
      {!channel?.members?.includes(user?.uid) && !channel?.isArchived && (
        <ModalButton
          ...
          onClick={joinChannel}
          text="Join"
          className="w-full hidden group-hover:inline-flex sm:ml-3 justify-center items-center py-1 px-4 border border-transparent text-sm font-bold rounded focus:outline-none focus:ring-4 focus:ring-blue-200 sm:w-auto sm:text-sm disabled:opacity-50 opacity-0 group-hover:opacity-100"
        />
      )}
      {channel?.members?.includes(user?.uid) && !channel?.isArchived && (
        <ModalButton
      ...
      {channel?.isArchived && (
        <ModalButton
          ...
          className="w-full hidden group-hover:inline-flex sm:ml-3 justify-center items-center py-2 px-4 border border-transparent text-sm font-bold rounded focus:outline-none focus:ring-4 focus:ring-blue-200 sm:w-auto sm:text-sm disabled:opacity-50 opacity-0 group-hover:opacity-100"
        />
      )}
    </li>
  );
      ...
      {loading && !value ? (
        ...
        >
          ...
          >
            ...
            <span className="text-sm th-color-for">{t("Create_a_channel")}</span>
          </li>
